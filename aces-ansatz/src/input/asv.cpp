...
namespace asv {
    ...
    asv::ioppar_flags parse_keyword_flags(input::input_block ib) {
        ...
        for (;it < it_end; ++it) {
            ...
            int hasmatched = 0;
            //loop over ASV list
            for(int ia=0; ia < asv::GlobalKeywords::get_max_asv(); ++ia) {
    ...
    asv::ioppar_flags init_keyword_flags()
    {
        ...
        int iASV;
        for (iASV=0;iASV < asv::GlobalKeywords::get_max_asv() ;iASV++) {
            f_flags.ioppar[iASV] = ASV_nl[iASV].ideflt;
        }
        return f_flags;
    }
    
    void asv_dump(asv::ioppar_flags f_flags)
    {
        std::regex np("#");
        int iArg;
        int i=0;
        ...
        for (iArg=0;iArg<asv::GlobalKeywords::get_max_asv();iArg++)
        {
    ...
    void asv_handle_proc(int index, std::string input_string_value, int output_keyword_value)
    {
